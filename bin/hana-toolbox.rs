...
#[derive(Parser)]
#[clap(name = "Hana Toolbox", about = "Utilities for Hana Ethereum client")]
struct Opt {
...
async fn download_headers(
    ...
    let env = Arc::new(hana::kv::new_database(&CHAINDATA_TABLES, &chain_data_dir)?);
...
fn open_db_rw(
    ...
    hana::kv::mdbx::MdbxEnvironment::<mdbx::NoWriteMap>::open_rw(
        mdbx::Environment::new(),
        &data_dir.chain_data_dir(),
        &CHAINDATA_TABLES,
    )
...
fn db_drop(data_dir: HanaDataDir, table: String) -> anyhow::Result<()> {
    let env = open_db_rw(data_dir)?;
...
fn read_block(data_dir: HanaDataDir, block_num: BlockNumber) -> anyhow::Result<()> {
    ...
    let header = hana::accessors::chain::header::read(&tx, block_num)?
        .ok_or_else(|| format_err!("header not found"))?;
    let body = hana::accessors::chain::block_body::read_without_senders(&tx, block_num)?
...
fn read_account_changed_blocks(data_dir: HanaDataDir, address: Address) -> anyhow::Result<()> {
    let env = open_db(data_dir)?;
