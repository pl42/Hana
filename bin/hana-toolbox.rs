...
pub enum OptCommand {
    ...
    HeaderDownload {
        #[structopt(flatten)]
        opts: hana::downloader::opts::Opts,
    },
}
...
async fn header_download(
    ...
    let chains_config = hana::sentry::chain_config::ChainsConfig::new()?;
    hana::downloader::opts::Opts::validate_chain_name(
        &opts.chain_name,
    ...
    let sentry_connector =
        hana::sentry::sentry_client_connector::SentryClientConnectorImpl::new(sentry_api_addr);

    let sentry_status_provider =
        hana::downloader::sentry_status_provider::SentryStatusProvider::new(chain_config.clone());
    let mut sentry_reactor = hana::sentry::sentry_client_reactor::SentryClientReactor::new(
    ...
    let stage =
        hana::stages::HeaderDownload::new(chain_config, sentry.clone(), sentry_status_provider);
    let db = hana::kv::new_database(&data_dir)?;
...
async fn check_table_eq(db1_path: PathBuf, db2_path: PathBuf, table: String) -> anyhow::Result<()> {
    let env1 = hana::MdbxEnvironment::<mdbx::NoWriteMap>::open_ro(
        mdbx::Environment::new(),
        &db1_path,
        Default::default(),
    )?;
    let env2 = hana::MdbxEnvironment::<mdbx::NoWriteMap>::open_ro(
        mdbx::Environment::new(),
        &db2_path,
        Default::default(),
    )?;
