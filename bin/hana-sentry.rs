...
use hana::sentry::{devp2p::*, eth::*, services::*, CapabilityServerImpl};
use anyhow::Context;
use cidr::IpCidr;
use clap::Parser;
...
#[derive(Debug, FromStr)]
pub struct Discv4NR(pub hana::sentry::devp2p::disc::v4::NodeRecord);
...
impl OptsDnsDisc {
    fn make_task(self) -> anyhow::Result<DnsDiscovery> {
        ...
        let dns_resolver = hana::sentry::devp2p::disc::dns::Resolver::new(Arc::new(
...
impl OptsDiscV4 {
    async fn make_task(self, secret_key: &SecretKey) -> anyhow::Result<Discv4> {
        ...
        let node = hana::sentry::devp2p::disc::v4::Node::new(
...
async fn main() -> anyhow::Result<()> {
    ...
    info!(
        "Node ID: {}",
        hex::encode(
            hana::sentry::devp2p::util::pk2id(&PublicKey::from_secret_key(SECP256K1, &secret_key))
                .as_bytes()
        )
    );
